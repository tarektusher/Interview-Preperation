...
void displayLinkedListRightToLeft(){
    struct node *temp = head;
    cout<<"Display Linked List from Right to Left"<<endl;
    while(temp != NULL){
        cout<<temp->num<<" ";
        temp = temp -> next;
    }
...
void displayLinkedListLeftToRight(node *temp){
    if(temp == NULL)return ;
    displayLinkedListLeftToRight(temp->next);
    cout<<temp->num<<" ";
}
int main()
{
    ...
    displayLinkedListRightToLeft();
    cout<<"Display Linked List from Left to Right"<<endl;
    displayLinkedListLeftToRight(head);
    return 0;
}
